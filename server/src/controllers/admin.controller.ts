...
export const getAdminMetrics = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const revenueMap = new Map(
      revenueAggregation.map((item) => [item._id, item.revenue]),
    );
  ...
  $group: {
    ...
    const orderMap = new Map(
      orderAggregation.map((item) => [item._id, item.orders]),
    );
    const orderData: OrderPoint[] = [];
    ...
    const enrollmentMap = new Map(
      enrollmentAggregation.map((item) => [item._id, item.enrollments]),
    );
